...
namespace Nerdbank.Streams
{
	...
	internal class ReadOnlySequenceStream : Stream, IDisposableObservable
	{
		...
		public override long Position
		{
			...
			set
			{
				Requires_Range(value >= 0, nameof(value));
				this.position = this.readOnlySequence.GetPosition(value, this.readOnlySequence.Start);
			}
		}

		private static void Requires_Range(bool ok, string paramName)
		{
			if (!ok)
		...
		private static void Verify_NotDisposed(ReadOnlySequenceStream s)
		{
			if (s.IsDisposed)
